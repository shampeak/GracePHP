...
class Bootstrap
{
    ...
    */
    ...
    public static function Ini()
    {
        ...
        //d(app('req')->get);        //D(app('req')->get);        //构建req数据流
        $get = app('req')->get;
...
$_file[] = $file;
        ...
        if(!method_exists($__controllerAction, 'behaviors')) {
            $controller->behavior = $controller->behaviors();       //根据behavior执行前后置的hook
        }

        if(!method_exists($__controllerAction, 'actions')) {
            $controller->actions = $controller->actions();       //根据behavior执行前后置的hook
        }
